  ...
  <div>
    <section class="pt-10 min-h-screen ">
      <ul class=" text-text-main flex flex-wrap h-fit gap-6 mb-8 w-full select-none justify-center  max-sm:text-sm">
        ...
        <li
          @click="() => fillteredBooks === 'science_fiction' ? fillteredBooks = '' : fillteredBooks = 'science_fiction'"
          :class="[fillteredBooks === 'science_fiction' ? 'bg-white! text-text-secondary!' : '']"
          class=" flex  h-fit items-center px-4 py-1 border rounded-full cursor-pointer ">
          <FontAwesomeIcon class="mr-2" :icon="faRobot" />
          <p>science fiction</p>
        </li>
        <li @click="() => fillteredBooks === 'mystery' ? fillteredBooks = '' : fillteredBooks = 'mystery'"
          :class="[fillteredBooks === 'mystery' ? 'bg-white! text-text-secondary!' : '']"
          class=" flex  h-fit items-center px-4 py-1 border rounded-full cursor-pointer ">
          <FontAwesomeIcon class="mr-2" :icon="faGhost" />
          <p>mystery</p>
        </li>
        <li @click="() => fillteredBooks === 'history' ? fillteredBooks = '' : fillteredBooks = 'history'"
          :class="[fillteredBooks === 'history' ? 'bg-white! text-text-secondary!' : '']"
          class=" flex  h-fit items-center px-4 py-1 border rounded-full cursor-pointer ">
      ...
      <div class="flex flex-wrap px-8 w-full gap-4 justify-center">
        <BookCardCom v-if="books && (!fillteredBooks || fillteredBooks === 'fantasy')" v-for="book in books.fantasy"
          :key="book.id" :book="book">
        </BookCardCom>
        <BookCardCom v-if="books && (!fillteredBooks || fillteredBooks === 'science_fiction')"
          v-for="book in books.science_fiction" :key="book.id" :book="book"
          @showInfos="(bookObj) => (bookDetail = bookObj)">
        </BookCardCom>
        <BookCardCom v-if="books && (!fillteredBooks || fillteredBooks === 'mystery')" v-for="book in books.mystery"
          :key="book.id" :book="book">
        </BookCardCom>
        <BookCardCom v-if="books && (!fillteredBooks || fillteredBooks === 'history')" v-for="book in books.history"
          :key="book.id" :book="book" </BookCardCom>
      </div>
