...
<template>
  ...
  <section class="mt-20 py-10 bg-Shark">
    <div class="container mx-auto sm:px-4">
      <h3 class="w-fit text-white text-4xl max-sm:mx-auto pt-8 font-bold">
        users reviews
        <span class="text-2xl text-[#888] font-normal"
          >({{ book.volumeInfo.ratingsCount || 0 }})</span
        >
      </h3>
      <button
        ref="toggleButton"
        @click="toggleAddReviewCom"
        id="part"
      ...
      <AddReviewCom v-if="showAddReviewCom" ref="addReviewCom" />

      <div class="slider flex overflow-x-scroll w-full mt-8 mx-auto">
        <ReviewBoxCom
          v-if="book.volumeInfo.ratingsCount"
          v-for="(review, index) in book.volumeInfo.ratingsCount"
          :key="index"
        ...
        >
          ...
          <button
            class="px-4 py-2 bg-primary mx-auto block bg-bg-main mt-10 text-white rounded-lg hover:bg-blue-600"
          >
            add new review
          </button>
        </div>
  ...
  <section class="py-10 mt-40 mx-auto bg-Shark">
    <div class="container mx-auto px-4">
      ...
      <div
        class="flex max-w-316 mx-auto snap-center slider w-full overflow-x-scroll gap-x-4 my-8"
      >
        <BookCardCom
          :class="` min-w-45 text-lg`"
