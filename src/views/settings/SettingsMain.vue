        ...
        <div class="mx-auto w-full max-w-6xl">
            ...
            <div class="grid gap-6 lg:grid-cols-[320px_1fr]">
                <aside class="space-y-6">
                    ...
                    <section class="rounded-lg border border-bg-section/20 bg-Shark p-6 ">
                        ...
                        <div class="mt-4 space-y-3">
                            <template v-for="item in settingGroups" :key="item.title">
                                <RouterLink v-if="item.to" :to="{ name: item.to, params: { id: userAuth.user?.uid } }"
                                    class="flex w-full items-start gap-3 rounded-lg border border-bg-section/20 bg-bg-section/5 p-4 text-left transition hover:border-bg-section/20 hover:bg-bg-section/8">
                                    <span
                                    ...
                                    <span class="min-w-0 flex-1">
                                        ...
                                        <span class="mt-1 block text-sm leading-5 text-bg-section/60">{{
                                            item.description
                                        }}</span>
                                    </span>
                                </RouterLink>
                ...
                <main class="space-y-6">
                    <section class="rounded-lg border border-bg-section/20 bg-Shark p-6 ">
                        <div class="flex text-white items-center gap-2 text-lg font-semibold">
                            <FontAwesomeIcon :icon="faShieldHalved" />
                            ...
                            <article v-for="item in settingGroups" :key="item.title"
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <RouterLink v-if="item.to"
                                        :to="{ name: item.to, params: { id: userAuth.user?.uid } }"
                    ...
                    <section class="rounded-lg border border-bg-section/20 bg-Shark p-6 ">
                        ...
                        <form class="mt-5 space-y-4" @submit.prevent>
                            <input v-model="supportTitle" type="text"
                                class="w-full  rounded-lg border border-bg-section/20 bg-bg-main/70 focus:border-bg-tertiary transition px-4 py-3 text-sm text-bg-section placeholder:text-bg-section/40"
                                placeholder="Title of the message" />
                            <textarea v-model="supportMessage" placeholder="Message body"
                                class="block min-h-52 w-full rounded-lg border border-bg-section/20 focus:border-bg-tertiary transition bg-bg-main/70 px-4 py-3 text-sm  text-bg-section placeholder:text-bg-section/40"></textarea>
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <p class="text-sm text-bg-section/55">Send feedback, report an issue, or ask for help
                                    with
                                    your account.</p>
                                <button @click="supportMessage = '', supportTitle = ''" type="submit"
